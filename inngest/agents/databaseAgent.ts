...
const saveToDatabaseTool = createTool({
  ...
  parameters: z.object({
    ...
    receiptId: z
      .string()
      .describe("The ID of the receipt to save to the database."),
    merchantName: z
      .string()
      .describe("The name of the merchant that the receipt is from."),
    ...
    receiptSummary: z.string().describe(
      `A summary of the receipt, including the merchant name, address, contact, transaction date, transaction amount, and currency. Include a human readable summary
        of the receipt. Mention both invoice number and receipt number if both are present. Include some key details about the items on the receipt, this is a special
        featured summary so it should include some key details about the items on the receipt with some context.`,
    ),
    currency: z.string(),
  ...
  handler: async (params, context) => {
    console.log("Saving to database", params);
    const {
      fileDisplayName,
      receiptId,
      merchantName,
